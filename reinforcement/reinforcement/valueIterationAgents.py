...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            ...
            xCount = util.Counter() #keeps utilities of each state

            for s in states:
                #utility of terminal state is 0
    ...
    def computeActionFromValues(self, state):
        ...
        bestAction = None
        bestValAction = -999999

        for a in actions:
            qVal = self.computeQValueFromValues(state,a)
            if qVal > bestValAction:
                bestValAction = qVal
                bestAction = a
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            ...
            if not self.mdp.isTerminal(state):
                actions = self.mdp.getPossibleActions(state)
                maxVal = max([self.getQValue(state,a) for a in actions])
        ...
        for s in self.mdp.getStates():
            if not self.mdp.isTerminal(s):
                for a in self.mdp.getPossibleActions(s):
                    for stateAndProb in self.mdp.getTransitionStatesAndProbs(s, a):
                        if stateAndProb[0] in predecessors:
                            predecessors[stateAndProb[0]].add(s)
                        else:
                            predecessors[stateAndProb[0]] = {s}
