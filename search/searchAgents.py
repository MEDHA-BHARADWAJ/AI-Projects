...
def cornersHeuristic(state, problem):
    ...
    for c in corners:
        if state[0] == c:
            return 0
        val+=pythogoras(state[0],c)
    
    s = problem.getSuccessors(state)
    cost = 0;
    for n in s:
        cost = cost + n[2]
    return abs(val-cost) # Default to trivial solution
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        x,y = state
        dist = PriorityQueue()
        for a in self.food.asList():
            dist.push(a, manhattanDistance(state,a))
